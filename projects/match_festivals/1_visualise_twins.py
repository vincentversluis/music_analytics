        ...
        [
            [
                ...
                if 
                    ...
                    not genre_of_interest 
                    or artist['genre'] == genre_of_interest
            ] 
            for festival 
            in festivals
        ]))
...
top_festival_twins = {
    ...
    for artists, festivals 
    in festival_twins.items()
...
top_festival_twins_sorted = sorted(top_festival_twins.items(), key=lambda x: x[1], reverse=True)[:top_n]
...
labels = [f"{a:>{name_chars}} & {b:<{name_chars}}" for (a, b), _ in top_festival_twins_sorted]
...
plt.grid(axis='x', color='grey', linestyle='--', linewidth=0.5)
plt.xlabel('Number of appearances')
plt.title(f'Top artists sharing a lineup with a {genre_of_interest.lower()} artist between 2022 and 2025')
plt.gca().invert_yaxis()
ax = plt.gca()
ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
...
# Sort and filter
top_festival_twins_sorted = sorted(top_festival_twins.items(), key=lambda x: x[1], reverse=True)
...
plt.title(f'Top artists sharing a line-up with {artist_of_interest} at between 2022 and 2025')
plt.gca().invert_yaxis()
ax = plt.gca()
ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
