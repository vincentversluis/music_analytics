...
lyrics_len_outlier_threshold = 0.99  # Threshold for removing suspiciously lengthy lyrics
...
songs_df = songs_df[[
    'artist', 
    'lyrics_length', 'lexical_diversity', 
    'sentiment'
    ]]
artist_agg_df = artist_agg_df[[
    'artist', 
    'perspective', 'directness',  # Aggregation seems to say more than details
    'emotion_anger', 'emotion_sadness', 'emotion_joy'
    ]]
...
# %% Nice aggregation
emotion_columns = ['perspective', 'directness', 'emotion_joy', 'emotion_anger', 'emotion_sadness']
...
for i, col in enumerate(emotion_columns):
    ...
    filtered = artist_agg_df[
        (artist_agg_df[col] > 0) & (artist_agg_df[col] < 1)
    ].copy()
