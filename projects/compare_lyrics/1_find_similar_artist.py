...
# %% FUNCTIONS
def find_similar_artists_by_lyrics(
    artist: str, X: np.ndarray, artists: np.ndarray, top_n: int | None = None
) -> list:
    ...
    # Display results
    return [
        (artist, float(round(dist, 3)))
        for i, (artist, dist) in enumerate(zip(nearest_artists, nearest_distances), 1)
    ]
