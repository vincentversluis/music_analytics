...
for artist in tqdm(artists, desc="Getting Last.fm data"):
    ...
    lastfm_data[artist]['similar_artists'] = pd.DataFrame(get_similar_artists(
        artist_name=artist, 
        lastfm_api_key=lastfm_api_key,
        limit=similar_artists_n
    ))
    # Listener count
...
for artist in artists:
    ...
    df_lastfm.rename(columns={'name': 'similar_artist', 'similarity': 'score_lastfm'}, inplace=True)

    df_metallum = df_metallum_all[df_metallum_all['artist'] == artist][['similar_artist', 'score']]
...
for ax, (artist, artist_data) in zip(axes, data.items(), strict=False):
    ...
    for _, row in top_similarities.iterrows():
        ...
        if name not in labeled:
            ax.scatter(row['rank_lastfm'], row['rank_metallum'], color='black', s=50, zorder=3)
            ax.annotate(name,
    ...
    for _, row in top_discrepancies.iterrows():
        name = row['similar_artist']
        if name not in labeled:
            ax.scatter(row['rank_lastfm'], row['rank_metallum'], color='dimgrey', s=50, zorder=3)
            ax.annotate(name,
                        (row['rank_lastfm'], row['rank_metallum']),
    ...
    # Legend
    legend_elements = [
        Line2D([0], [0], marker='o', color='black', label='Top similarity', markersize=8),
        Line2D([0], [0], marker='o', color='dimgrey', label='Top discrepancy', markersize=8),
        Line2D([0], [0], marker='o', color='royalblue', label='Other artists', markersize=8)
    ]
